...
function [masks, scores] = cpmc(exp_dir, img_name, diversify_const, segm_pars)
    ...
    DefaultVal('*ranker_file', '''attention_model_fewfeats_lambda_10.00_trainval.mat'''); 
    ...
    % of VOC2010.
    ranker_file = 'simple_model_fewfeats_lambda_10.00_trainval.mat';
    load([exp_dir '/MySegmentRankers/' ranker_file]);
    
    t = tic();
    disp('Starting feature extraction.');
    % --- segment_measurements in ranker ---
    D = get_features(exp_dir, img_name, segm_pars.name, masks, ...
    ...
    if(iscell(scaling)) % --- scaling in ranker ---
    ...
    Feats = D_scaled;
    [Feats] = SvmSegm_mix_linear_and_random_features(Feats, ...
        w.kernels, ... % --- w.kernels in ranker ---
        scaling_type, ...% --- scaling_type in ranker ---
        w.scaling_RF, ...% --- w.scaling_RF in ranker ---
        dims, ...
        [], ...% gamma
        w.N_OUT_DIMS, ... % --- w.N_OUT_DIMS in ranker ---
        w.obj, ... % --- w.obj in ranker ---
        w.merge_kernels ... % --- w.merge_kernels in ranker ---
    );
    % --- w.weights in ranker ---
...
function D = get_features(exp_dir, img_name, mask_type, masks, feat_types)
    ...
    % simple & gestalt
    D_1 = SvmSegm_compute_segm_feats_nofiles(exp_dir, img_name, masks, ...
        segm_feats(sf_in));    
    t_segm_feats1 = toc(t1);
    fprintf('Time getting first set of features: %f\n', t_segm_feats1);
    
    t2 = tic();
    recog_feats_in = setdiff(1:numel(feat_types), sf_in);%[3 4 5 6]
