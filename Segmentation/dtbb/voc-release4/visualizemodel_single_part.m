function hfs = visualizemodel(model, components, layers)
...
for i = components
  for layer = layers
      if nargout==1
          hfs = visualizecomponent(model, i, length(layers)*length(components), k, layer);
      else
...
function hfs = visualizecomponent(model, c, nc, k, layer)
