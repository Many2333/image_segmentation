function [dets, boxes, info,times,model] = gdetect(pyra, model, thresh, bbox, overlap)
...
for j = 1:length(r.rhs)
  ...
  for i = startlevel:length(s)
    ...
    mess{i}{j} = stmp;
    offs{i}{j} = [oy+1,oy+sz(1),ox+1,ox+sz(2)];
  end
end
model.rules{r.lhs}(r.i).score = score;
model.rules{r.lhs}(r.i).mess  = mess;
model.rules{r.lhs}(r.i).offs  = offs;
...
for s = model.symbols
  if s.type == 'T'
      if model.dofast
          filters{i} = single(model.filters(s.filter).w);
      else
...
for level = levels
  ...
  if model.dofast
      r = fconvsse(single(pyra.feat{level}), filters, 1, length(filters));
  else
