...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs>D_IND)
    {
        ...
        if (mxGetNumberOfElements(prhs[D_IND])>0)
        {
            ...
            //if (epsilon>0)
             //   fct  = fastSqrt(1/epsilon);
        }
    }
        
    if (0)
    mexPrintf("h_s %i h_e %i v_s %i v_e %i Nh %i  Npixels %i Nmodels %i Nwords %i Nmodels_all %i NmNw %i Nv %i Nh %i\n",\
               h_s, h_e, v_s, v_e, Nh, Npixels, Nmodels, Nwords, Nmodels_all,NmNw,Nv,Nh);
    
    code_setup
            
   // mexPrintf("1 %i 2 %i 3 %i 4 %i 5 %i 6 %i \n",offi[0],offi[1],offi[2],offi[3],offi[4],offi[5]);
